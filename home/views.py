...
def userimageUpdate(request):
    if request.method == "POST":
        ...
        if "defa" not in str(userprofile.userimage):
            cloudinary.uploader.destroy(
...
def userimageRemove(request):
    ...
    if "defa" not in str(userprofile.userimage):
        cloudinary.uploader.destroy(
            userprofile.userimage.public_id, invalidate=True)
        print(userprofile.userimage)
    userprofile.userimage = 'image/upload/v1615483987/promedia/profiles/defauly_wodtjs.jpg'
    userprofile.save()
    return redirect("/profile/" + request.user.username)
